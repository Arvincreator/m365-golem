...
function buildM365PlanObservation(input = {}) {
    ...
    return `[GOLEM_OBSERVATION]\n${JSON.stringify(payload, null, 2)}\n[/GOLEM_OBSERVATION]\n` +
        ...
        `Return the next revision of [GOLEM_PLAN]. If it remains running, also emit one bounded [GOLEM_ACTION] block with one or more ordered actions for the current step. ` +
        ...
        `In [GOLEM_REPLY], describe only useful progress or results in plain user language; never expose internal workflow names, protocol labels, raw commands, diagnostics, JSON field names, or error codes.`;
}

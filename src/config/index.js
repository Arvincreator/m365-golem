...
const normalizeBackend = (value) => {
    ...
    if (backend === 'm365' || backend === 'copilot' || backend === 'copilot-web') return 'm365-web';
    if (backend === 'gemini' || backend === 'ollama' || backend === 'lmstudio' || backend === 'perplexity' || backend === 'm365-web') return backend;
    return 'gemini';
};
...
const CONFIG = {
    ...
    M365_COPILOT_URLS: (process.env.M365_COPILOT_URLS || '').split(',').map(u => cleanEnv(u, true)).filter(u => u),
    M365_POC_SAFE_MODE: parseBooleanEnv(process.env.M365_POC_SAFE_MODE, true),
    ...
    M365_RESPONSE_TIMEOUT_MS: Number(cleanEnv(process.env.M365_RESPONSE_TIMEOUT_MS)) || 300000,
...
// M365 Web POC 安全模式只處理使用者明確送出的純文字，不允許背景自主喚醒或反思回合。
...
const reloadConfig = () => {
    ...
    const newM365Urls = (process.env.M365_COPILOT_URLS || '').split(',').map(u => cleanEnv(u, true)).filter(u => u);
    CONFIG.M365_COPILOT_URLS.length = 0;
    CONFIG.M365_COPILOT_URLS.push(...newM365Urls);
    CONFIG.M365_POC_SAFE_MODE = parseBooleanEnv(process.env.M365_POC_SAFE_MODE, true);
    ...
    CONFIG.M365_RESPONSE_TIMEOUT_MS = Number(cleanEnv(process.env.M365_RESPONSE_TIMEOUT_MS)) || 300000;

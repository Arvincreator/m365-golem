...
async function main() {
    ...
    try {
        ...
        await page.getByRole('button', { name: '來源與執行', exact: true }).click();
        await page.getByRole('dialog').waitFor();
        await page.waitForTimeout(300);
        assert.match(await page.getByRole('dialog').innerText(), /受阻/);
        assert.match(await page.getByRole('dialog').innerText(), /已略過/);
        assert.match(await page.getByRole('dialog').innerText(), /1\/3/);
        await page.screenshot({ path: path.join(output, 'inspector-1366.png'), fullPage: true });
        await page.keyboard.press('Escape');
        await page.getByRole('dialog').waitFor({ state: 'hidden' });
        assert.equal(await page.getByRole('dialog').count(), 0);
        assert.equal(await page.getByRole('button', { name: '來源與執行', exact: true }).evaluate(node => node === document.activeElement), true);

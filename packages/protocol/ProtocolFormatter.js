...
- When a suitable Skill or MCP is not visible, discover it using {"action":"command","parameter":"golem-check tools <specific task description>"}. The host returns freshly selected enabled routes and their exact usage guides without executing them. Treat these guides like the per-turn routing context, not as proof of connection. Do not fabricate names or read raw MCP configuration/credentials to discover tools. If discovery finds nothing, inspect relevant local runtime alternatives or explain the specific unverified capability rather than claiming universal inability.

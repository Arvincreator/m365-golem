...
- 不召回或注入 GOLEM 的全域記憶、Wiki 或整本技能庫；只載入目前專案的 `AGENTS.md`／專案記憶、使用者本輪選定的知識來源，以及向量路由命中的少量工具與節錄使用指南。
...
- 提出實際工具動作時，Copilot 對使用者只顯示「我正在確認，請稍候…」；內部流程與技術名稱不顯示在一般對話中，最終再以白話回報真實結果。
- 不啟動背景自主喚醒或反思回合。

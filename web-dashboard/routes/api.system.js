...
function normalizeBackend(backendRaw) {
    ...
    if (backend === 'gemini' || backend === 'ollama' || backend === 'lmstudio' || backend === 'perplexity' || backend === 'm365-web') {
        return backend;
    }
    return 'gemini';
}
...
module.exports = function registerSystemRoutes(server) {
    ...
    router.post('/api/system/backend/test', requireSystemConfigUpdate, async (req, res) => {
        try {
            ...
            return res.status(400).json({
                success: false,
                error: 'Connection test currently supports only ollama and lmstudio backends. Web backends require a visible human-authenticated browser check.'
            });
        } catch (e) {
    ...
    router.get('/api/system/update/check', async (req, res) => {
        try {
            ...
            const isM365SafeMode = ConfigManager.CONFIG.GOLEM_BACKEND === 'm365-web'

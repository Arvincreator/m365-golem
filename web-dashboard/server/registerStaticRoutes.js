  ...
  <main>
    ...
    if (!isDevMode) {
        server.app.use(express.static(publicPath, {
            extensions: ['html'],
            // Next static export creates both `skills.html` and a `skills/`
            // payload directory. Do not let express.static redirect to the
            // directory before the explicit dashboard route serves the page.
            redirect: false,
            setHeaders: (res) => {
                res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');

...
const GolemContext = createContext<GolemContextType>({
    ...
    isSingleNode: true,
    version: `v${process.env.NEXT_PUBLIC_GOLEM_VERSION || "9.6.19"}`,
    allowRemote: false,
    localIp: "127.0.0.1",
    dashboardPort: 3000,
    backend: "gemini",
...
export function GolemProvider({ children }: { children: React.ReactNode }) {
    ...
    const [version, setVersion] = useState(`v${process.env.NEXT_PUBLIC_GOLEM_VERSION || "9.6.19"}`);
    ...
    const fetchSystemStatus = async () => {
        ...
        try {
            const data = await apiGet<{ version?: string; golemBackend?: string }>(
                apiUrl("/api/system/config"),
                undefined,
                { profile: "none" }
            );
            if (data?.version) setVersion(`v${data.version}`);
            if (data?.golemBackend) setBackend(data.golemBackend);
        } catch {

                    ...
                    <div className="bg-card border border-border rounded-xl p-5 shadow-sm">
                        ...
                        <div className="flex flex-col mb-4">
                            ...
                            >
                                ...
                                <option value="m365-web">Microsoft 365 Copilot Chat Web (POC)</option>
                        ...
                        {env.GOLEM_BACKEND === "m365-web" && (
                            <div className="bg-blue-500/5 p-4 rounded-xl border border-blue-500/20 space-y-4 animate-in zoom-in-95">
                                <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-100 leading-relaxed">
                                    此模式直接操作可見的 Microsoft 365 Copilot Chat 網頁，不使用 Copilot Chat API。登入與 MFA 必須由你在 Edge 視窗完成；POC 預設不把對話寫入 GOLEM 記憶或歷史、不執行模型動作，也不會在啟動時自動送出訊息。Microsoft 365 與 Edge 本身的保存政策仍然適用。
                                </div>
                                <SettingField
                                    label="Microsoft 365 Copilot Chat URL"
                                    keyName="M365_COPILOT_URLS"
                                    placeholder="https://m365.cloud.microsoft/chat"
                                    desc="只建議使用正式 m365.cloud.microsoft 網址。"
                                    value={env.M365_COPILOT_URLS || ""}
                                    onChange={(val) => onChangeEnv("M365_COPILOT_URLS", val)}
                                />
                                <SettingField
                                    label="Dedicated browser profile"
                                    keyName="PLAYWRIGHT_PROFILE"
                                    placeholder="m365-work"
                                    desc="建議使用獨立工作設定檔，避免與日常 Edge 視窗互相鎖定。"
                                    value={env.PLAYWRIGHT_PROFILE || ""}
                                    onChange={(val) => onChangeEnv("PLAYWRIGHT_PROFILE", val)}
                                />
                                <SettingSelectField
                                    label="M365 POC safe mode"
                                    desc="建議維持 true；false 仍不代表已通過正式環境核准。"
                                    value={env.M365_POC_SAFE_MODE || "true"}
                                    onChange={(val) => onChangeEnv("M365_POC_SAFE_MODE", val)}
                                    options={[
                                        { value: "true", label: "true — 純文字安全 POC（建議）" },
                                        { value: "false", label: "false — 進階實驗（未核准）" },
                                    ]}
                                />
                            </div>
                        )}

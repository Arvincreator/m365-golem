...
type BackendMode = "gemini" | "m365-web" | "ollama" | "lmstudio";
    ...
    useEffect(() => {
        const loadConfig = async () => {
            try {
                ...
                else if (data.golemBackend === "m365-web") setBackend("m365-web");
                    ...
                    <div className="bg-card/80 backdrop-blur-sm border border-border rounded-2xl p-6 shadow-xl relative overflow-hidden">
                        ...
                        <div className="mb-5">
                            ...
                            >
                                ...
                                <option value="m365-web">Microsoft 365 Copilot Chat Web (POC, no API)</option>
                            ...
                            <p className="text-xs text-muted-foreground/60 mt-1.5">
                                Ollama / LM Studio 模式不需瀏覽器登入；Gemini 與 M365 模式使用 Browser-in-the-Loop。
                            </p>
                        </div>

                        {backend === "m365-web" && (
                            <div className="mb-5 bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-xs text-amber-100 leading-relaxed">
                                初始設定只會選擇 M365 Web 後端，不會測試或代替登入。請完成設定後，到進階設定確認使用獨立的 <code>m365-work</code> 瀏覽器設定檔；登入與 MFA 必須在可見的 Edge 視窗由使用者完成。預設安全模式不使用 Copilot Chat API、不把對話寫入 GOLEM 記憶或歷史、也不執行模型動作；Microsoft 365 與 Edge 本身的保存政策仍然適用。
                            </div>
                        )}

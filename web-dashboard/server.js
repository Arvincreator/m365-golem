...
const registerM365WorkspaceRoutes = require('./routes/api.m365-workspace');
...
class WebServer {
    ...
    init() {
        ...
        this.app.use(buildM365OnlyApiGuard());

        // M365 is a web backend, not a reduced Golem edition. Keep the original
        // tool-management surface and remove only the explicitly retired modules.
        const routeFactories = [
            registerM365WorkspaceRoutes,
    ...
    startServer() {
        ...
        const tryListen = (port, attempt = 1) => {
            ...
            this.server.listen(port, this.allowRemote ? '0.0.0.0' : '127.0.0.1');
        };
    ...
    broadcastLog(data) {
        ...
        const isM365WorkspaceEvent = ConfigManager.CONFIG.GOLEM_BACKEND === 'm365-web'
            && data
            && data.conversationId;
        if (isM365WorkspaceEvent) {
            if (this.io) {
                this.io.emit('log', {
                    time: data.time,
                    type: data.type,
                    golemId: data.golemId,
        ...
        const shouldRetain = !(data && data.transient === true);
        if (shouldRetain) {
